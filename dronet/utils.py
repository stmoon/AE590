...
class DroneDataGenerator(ImageDataGenerator):
    ...
    """
    def flow_from_directory(self, directory, target_size=(500,700),
            crop_size=(500,700), color_mode='grayscale', batch_size=32,
...
class DroneDirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
            target_size=(500,700), crop_size = (500,700), color_mode='grayscale',
            ...
            for frame_number, fname in enumerate(sorted_files):
                ...
                for extension in self.formats:
                    if fname.lower().endswith('.' + extension):
                        #print(fname.split('_')[-1].replace('.png',''))
                        angle = float(fname.split('_')[-1].replace('.txt',''))
                        is_valid = True
                        break
                if is_valid:
                    absolute_path = os.path.join(root, fname)
                    self.filenames.append(os.path.relpath(absolute_path, self.directory))
                    self.ground_truth.append(angle)
                    #print(angle, os.path.relpath(absolute_path, self.directory))
                    self.exp_type.append(1)         ## steering angle(exp_type =1 )
                    self.samples += 1
...
def hard_mining_entropy(k):
    ...
    def custom_bin_crossentropy(y_true, y_pred):
        ...
        if n_samples_coll == 0:
            ...
            hard_l_coll = tf.multiply(hard_l_coll, 0)
